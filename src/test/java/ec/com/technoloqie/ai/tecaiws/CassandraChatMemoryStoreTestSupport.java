...
abstract class CassandraChatMemoryStoreTestSupport {
	 ...
	 protected static CassandraChatMemoryRepository chatMemoryStore;
	...
	void shouldConnectToDatabase() {
		...
		assertThat(chatMemoryStore.getCassandraSession().getMetadata().getKeyspace(KEYSPACE)).isPresent();
    ...
    void shouldCreateChatMemoryStore() {
        ...
        assertThat(chatMemoryStore.getCassandraSession()
                ...
                .getTable(CassandraChatMemoryRepository.DEFAULT_TABLE_NAME)).isPresent();
	...
	void shouldInsertItems() {
		...
		ChatMemory chatMemory = MessageWindowChatMemory.builder().chatMemoryStore(chatMemoryStore).maxMessages(100)
	...
	abstract CassandraChatMemoryRepository createChatMemoryStore();
}
